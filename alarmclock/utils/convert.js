...
const Temporal = require('@js-temporal/polyfill').Temporal.Now;

const convert = (time) => {
  const t = time || Temporal.plainTimeISO();
  let resultTime = '';
  ...
  resultTime += ':';
  // Get min
  if (minute < 10) resultTime += `0${minute}`;
  if (minute > 9) resultTime += String(minute);
  resultTime += ':';
  // Get sec
  if (second < 10) resultTime += `0${second}`;
